...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      ...
      subject(:game_guess_two) { described_class.new(5, 9, number_guessing).make_guess }

      it 'returns 7' do
        expect(game_guess_two).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do

      subject(:game_guess_three) { described_class.new(8, 9, number_guessing).make_guess }

      it 'returns 8' do
        expect(game_guess_three).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do

      subject(:game_guess_four) { described_class.new(0, 3, number_guessing).make_guess }

      it 'returns 1' do
        expect(game_guess_four).to eq(1)
      end
    end

    context 'when min and max both equal 3' do

      subject(:game_guess_five) { described_class.new(3, 3, number_guessing).make_guess }

      it 'returns 3' do
        expect(game_guess_five).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    ...
    let(:correct_number) { double('correct_number', value: 4) }

    context 'when guess and random_number are equal' do

      subject(:game_won) { described_class.new(0, 9, correct_number, 4) }
    ...
    context 'when guess and random_number are not equal' do
      ...
      subject(:game_not_won) { described_class.new(0, 9, correct_number, 5) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        expect { edge_case_game.update_range }.to change { edge_case_game.min }.from(5).to(8)
      end

      it 'does not update max' do
        expect { edge_case_game.update_range }.not_to change { edge_case_game.max }
      end
    end
  end
end
