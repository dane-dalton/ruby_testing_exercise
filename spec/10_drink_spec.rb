...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_matcha) { described_class.new('matcha', 16) }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_matcha.type).to be('matcha')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(my_matcha.ounces).to be(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_pbr) { described_class.new('PBR', 16) }

      # remove the 'x' before running this test
      it 'is full' do
        expect(my_pbr).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_pbr) { described_class.new('PBR', 0) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(my_pbr).not_to be_full
      end
    end
  end
end
